...
- Bindings: `API_KEYS` + provider token cache (KV), `PUSH_RATE_LIMITER` / `REGISTER_RATE_LIMITER` / `ADMIN_RATE_LIMITER` (native rate limiting), `USAGE_ANALYTICS` (Analytics Engine), `RELAY_COORDINATOR` (Durable Object). Each Worker re-exports `RelayCoordinator` so the runtime can find the class the binding names.
...
- **`ADMIN_SECRET` is rate limited before it is compared, and has a length floor** (`MIN_ADMIN_SECRET_LENGTH`). Constant-time comparison stops the secret leaking a character at a time; it does nothing about retrying, and this endpoint mints and revokes every key the relay honours. A secret below the floor authenticates nobody, including its holder, and every credential denial is the same bare 401.
