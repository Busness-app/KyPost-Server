...
- On a fresh install `bootstrap.sh` must produce a valid `admin.env` with a non-empty Argon2id hash (`$argon2id$` prefix) owned by `kypost`; on an install with `users.json` or `admin.env` present it must leave both untouched
