...
// TestLeafSPKIPinMatchesTheCertificatesPublicKey checks the pairing link's pin
// against an independently marshalled SubjectPublicKeyInfo — the same bytes
...
func TestLeafSPKIPinMatchesTheCertificatesPublicKey(t *testing.T) {
	dir := t.TempDir()
...
// termination here means no pin, and the app falls back to trust-on-first-use
...
// TestPairingPublishesThePinOnlyWhenWeServeTheCertificate covers the guard on
// the pairing response. Publishing a pin we do not serve is worse than
// publishing none: the app fails closed on a pin that doesn't match, so a
// deployment terminating TLS in a reverse proxy would have every new pairing
// break rather than fall back to trust-on-first-use.
func TestPairingPublishesThePinOnlyWhenWeServeTheCertificate(t *testing.T) {
	dir := t.TempDir()
	certPath := filepath.Join(dir, "c.pem")
	keyPath := filepath.Join(dir, "c.key")
	writeKeypair(t, certPath, keyPath, "relay.example.com")
	...
	t.Run("published when this process terminates TLS", func(t *testing.T) {
		cfg, err := newTLSConfig(certPath, keyPath)
		if err != nil {
			t.Fatalf("newTLSConfig: %v", err)
		}
		pin, ok := pairingPin(t, func(s *Server) {
			s.serverBaseURL = "https://relay.example.com"
			s.tlsConfig = cfg
		})
		if !ok {
			t.Fatal("no tlsPin in the pairing response; the app would pair over TOFU")
		}
		if want := leafSPKIPin(cfg); pin != want {
			t.Errorf("tlsPin = %q, want %q", pin, want)
		}
	})

	t.Run("absent behind a TLS-terminating proxy", func(t *testing.T) {
		if pin, ok := pairingPin(t, func(s *Server) {
			s.serverBaseURL = "https://relay.example.com"
		}); ok {
			t.Errorf("tlsPin = %q with no certificate of our own", pin)
		}
	})
	...
	t.Run("absent on a plain-http base URL", func(t *testing.T) {
		...
		if pin, ok := pairingPin(t, func(s *Server) {
			s.serverBaseURL = "http://relay.example.com"
			s.tlsConfig = cfg
		}); ok {
