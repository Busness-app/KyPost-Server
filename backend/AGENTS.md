...
- **`failureLockout`'s hard-cap eviction drops LOCKED entries before partial strike records.** Stage 2 of `sweepIfCrowdedLocked` sorts locked-first, then by soonest expiry. Sorting on `lockedUntil` alone put unlocked entries first — their `lockedUntil` is the zero `time.Time`, which orders before every real timestamp — so flooding past `loginLockoutHardCap` erased every real key's 1-of-3 and 2-of-3 progress while keeping the attacker's manufactured lockouts, and the three-strikes lockout stopped engaging for anybody. That is the same bypass `lastSeen` closes in stage 1, so the two stages must agree. Locked entries are also the right thing to drop on the merits: evicting one forgives a cooldown early, evicting a partial record disables the control. Pinned by `TestCrowdedTableEvictsLockedBeforePartialStrikes` (hard cap) alongside `TestCrowdedTableDoesNotErasePartialStrikes` (sweep threshold)
